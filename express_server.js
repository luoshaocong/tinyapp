...
app.get("/urls/:id", (req, res) => {
  ...
  if (doesUserExist(userId)) {
    res.status(401).send('plase log in first ');
    return;
  }

  if (userIsOwner(urlDatabase, urlID, userId)) {
    res.status(403).send('you don own that url');
    return;
  }
...
app.post("/login", (req, res) => {
  ...
  res.status(403).send('password or email not correct');
});
...
app.post("/register", (req, res) => {
  ...
  if (email === users.email) {
    res.status(400).send('acacount exist');
